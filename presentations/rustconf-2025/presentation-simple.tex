...
\textbf{Architectural Decision:} Rust's ownership model provides memory safety\\guarantees without sacrificing performance
\end{frame}
...
\textbf{What We Built}
\begin{itemize}
\item Core REPL loop with robust error handling
\item Signal handling ensures graceful interruption (Ctrl+C)
\item Asynchronous architecture from inception
\item Type-safe error handling prevents runtime failures
\end{itemize}
...
impl SecuritySandbox {
    pub fn validate_path(&self, path: &Path) -> Result<()> {
        // Defense in depth: TOCTOU-safe + capability checks
        let canonical = fs::canonicalize(path)?;
        // Prevent directory traversal attacks
        if !canonical.starts_with(&self.workspace) {
            security_audit_log!("Path traversal attempt: {:?}", path);
...
\textbf{Challenge:} AI models require excessive tokens for large file editing

\textbf{Solution:} Ed protocol - a time-tested, deterministic editing interface
\begin{lstlisting}
...
// Result: 90% reduction in token consumption
\end{lstlisting}
\end{frame}
...
\item \textbf{Verification:} Formal verification with Prusti/Kani
\item \textbf{Deployment:} WASM sandbox with seccomp-bpf filtering
\end{enumerate}
...
\item Zero security incidents (SOC 2 Type II compliant, penetration tested)
...
\begin{itemize}
\item 45 contributors from 12 countries (\faGlobe)
\item 200+ issues closed (avg response time: 6 hours)
\item 500+ stars on GitHub (\faStar)
\item Active Discord with 250+ members
\item Weekly community calls
\item Special thanks to FreeBSD ports maintainers!
\end{itemize}
\end{frame}
...
\column{0.5\textwidth}
\textbf{Join Our Community}
\begin{itemize}
\item \faDiscord~Discord: discord.gg/gemini-repl
\item \faTwitter~Twitter: @gemini\_repl
\item \faMastodon~Mastodon: @gemini\_repl@hachyderm.io
\item \faEnvelope~Email: team@gemini-repl.org
\item \faCalendar~Weekly calls: Thursdays 3PM PT
\end{itemize}
\end{columns}

\begin{center}
\Large
\textbf{Thank you RustConf Community!}

\normalsize
\faGift~Stickers and t-shirts available after the talk\\Join us for the hallway track discussion!
\end{center}
\end{frame}
